...
are packed into a fixed set of 256 `shard-{nn}.json` buckets keyed by party
id, because Cloudflare Pages caps a deployment at 20,000 **files** (not
bytes) and ~2,200 one-kilobyte party files cost as much of that budget as
2,200 large ones. Sharding cut the deployment from ~8,600 files to ~6,700.
