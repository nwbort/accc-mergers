...
function Mergers() {
  ...
  // Window virtualiser: renders only the cards currently in (or near) the viewport,
  // keeping the DOM small even when scrolling through hundreds of mergers. Uses the
  // window as the scroll container so page-level scrolling is unchanged.
  ...
  // Trigger the next page once the user has scrolled into the last few items.
  useEffect(() => {
    if (!hasMore || virtualItems.length === 0) return;
    const last = virtualItems[virtualItems.length - 1];
    if (last.index >= visibleMergers.length - 5) {
      setPage((p) => p + 1);
    }
  }, [hasMore, virtualItems, visibleMergers.length]);
  ...
  }, [visibleMergers, selectedIndex, navigate, rowVirtualizer]);
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
        ...
        {/* Mergers List (virtualised) */}
        <div
          ref={listRef}
          style={{ height: `${totalSize}px`, position: 'relative' }}
        >
          {virtualItems.map((virtualItem) => {
            const merger = visibleMergers[virtualItem.index];
            ...
            return (
              ...
              >
                <div
                  data-merger-index={virtualItem.index}
                  role="link"
                  tabIndex={0}
