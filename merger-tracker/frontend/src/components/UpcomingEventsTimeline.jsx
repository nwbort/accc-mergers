...
const DEFAULT_EVENT_TYPE = {
  label: 'Event',
  Icon: FaGavel,
  tile: 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300',
  chip: 'bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-gray-200/60',
};
...
function getUrgency(daysRemaining) {
  ...
  return { dot: 'bg-primary', ring: 'ring-primary/10', text: 'text-gray-900 dark:text-gray-100' };
}
          ...
          return (
            <li key={day.date.slice(0, 10)} className="relative flex gap-3 sm:gap-4">
              ...
              <div className="relative flex w-3 flex-none justify-center">
                {!isLast && (
                  <span
                    className="absolute top-2 bottom-0 w-px bg-gray-200 dark:bg-gray-700"
                    aria-hidden="true"
                  />
                )}
                <span
              ...
              <div className={`min-w-0 flex-1 ${isLast ? '' : 'pb-6'}`}>
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">{formatWeekday(day.date)}</span>
                </div>
                    ...
                    return (
                      <li key={`${event.merger_id}-${event.date}-${event.type}`}>
                        <Link
                          ...
                          className="group relative flex items-center gap-3 rounded-xl -mx-2 px-2 py-2 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800"
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100 transition-colors group-hover:text-primary dark:group-hover:text-accent-light">
                              {event.merger_name}
                            </div>
